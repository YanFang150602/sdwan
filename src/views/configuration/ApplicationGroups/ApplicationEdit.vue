<template>
  <div>
    <a-modal
      v-model="visible"
      title="Edit Custom Application Group"
      on-ok="handleOk"
      width="300px"
      v-drag
    >
      ...
      <a-form-model :rules="rules" :model="form.address" ref="ruleForm">
        <div class="title">
          ...
          <AddModle
            ...
            :list="AppData"
            v-model="form.address.predefinedApps"
          />
        </div>
      </a-form-model>
    </a-modal>
...
<script>
import { GroupCheck, GroupEdit } from 'apis/applicationGroups';
...
export default {
  ...
  methods: {
    async showModalEdit(data) {
      ...
      this.check.name = data.rowData.name;
      this.check.objectName = this.deviceName;
      this.check.objectType = this.objectType;
      this.check.orgName = this.organization;

      const res = await GroupCheck(this.check);
      console.log(res);
      this.form.address.name = res.result.name;
      this.form.address.description = res.result.description;
      this.form.address.predefinedApps = res.result.predefinedApps;
    },
    async getPredefined() {
    ...
    async createdService() {
      ...
      const res = await GroupEdit(this.form.address);
      console.log(res);
      if (res.message === 'Success') {
        this.visible = false;
