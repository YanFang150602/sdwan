  ...
  <div>
    <a-modal v-model="visible" title="Confirm Delete" on-ok="handleOk" :maskClosable="false">
      <template slot="footer">
        <a-button key="submit" type="primary" @click="handleOk" style="baclGround:#a7d054">Yes</a-button>
...
<script>
import { appGroupsDelete } from 'apis/applicationGroups';
...
export default {
  ...
  methods: {
    ...
    async handleOk() {
      ...
      //   this.loading = false;
      // }, 3000);
      const res = await appGroupsDelete(this.GroupDelete);
      console.log(res);
