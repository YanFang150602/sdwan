...
Axios.interceptors.request.use(
  config => {
    ...
    if (window.$vm.$store.state.common.token) {
      config.headers['access-token'] = window.$vm.$store.state.common.token;
    }
  ...
  response => {
    ...
    switch (response.data.status) {
      ...
      case '20308':
        ...
        break;
      case '001_1000_0011':
        jumpToLogin('001_1000_0011');
        break;
      // 对响应数据做点什么
      default:
        if (response.status === 200 || axiosBaseUrl.delPrefix) {
...
const $http = {
  ...
  patch(api, params, opts = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (api.includes('/other-url-name/') && !axiosBaseUrl.delPrefix) {
        url += '/other-url-name';
      } else {
        url += '';
      }
